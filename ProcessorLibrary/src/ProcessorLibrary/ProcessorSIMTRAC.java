...
public class ProcessorSIMTRAC extends Thread 
{	
	...
	private void AbrirDB() {
		try {
			...
			System.err
					.println("MSG: ProcessorSIMTRAC: AbrirDB: Conexion a la base de datos abierta");

		} catch (SQLException e) {
			System.err.println("SQLException: ProcessorSIMTRAC: AbrirDB: "
					+ e.toString());
			CerrarDB();
		} catch (Exception EXC) {
			System.err.println("Exception: ProcessorSIMTRAC: AbrirDB: "
					+ EXC.toString());
			CerrarDB();
		} catch (Throwable T) {
			System.err.println("Throwable: ProcessorSIMTRAC: AbrirDB: ");
			T.printStackTrace();
			CerrarDB();
		} finally {
			if (ConexionDB == null) {
	...
	private void AbrirArchivo(String Nombre) {
		try {
			...
			if (FILE.exists()) {
				System.err
						.println("MSG: ProcessorSIMTRAC: AbrirArchivo: Archivo "
			...
			System.err.println("Exception: ProcessorSIMTRAC: AbrirArchivo: "
					+ e.toString());
		}
	}
	...
	public void run() 
	{
		System.err.println("MSG: ProcessorSIMTRAC: Run: Iniciando ejecución");
		...
		if (DBParameters != null) 
		{
			...
			try 
			{
				System.err.println("MSG: ProcessorSIMTRAC: Run: Consultando Usuarios asociados a TAURUS");
				...
				System.err.println("MSG: ProcessorSIMTRAC: Run: Consulta finalizada en "
				...
				while (rs1.next()) 
				{
					...
					if (UsuarioProcesando != null && Usuario.compareTo(UsuarioProcesando) != 0 && RAFILE != null) 
					{
						...
						if( RegistrosLeidos > 0)
							...
							System.err.println("MSG: ProcessorSIMTRAC: No hay archvio para transmitir " );
					}
					...
					*/
					...
					while (rs2.next()) 
					{
						if ( RAFILE == null) 
						{
							AbrirArchivo(Usuario);
							UsuarioProcesando = Usuario;
							System.err.println("MSG: ProcessorSIMTRAC: Inicio de proceso para usuario " + UsuarioProcesando);
						}
				...
				{
					System.err.println("MSG: ProcessorSIMTRAC: Run: No hay archvio para transmitir " );
				}

			}catch (SQLException SE) 
			{
				System.err.println("IOException: ProcessorSIMTRAC: Run: " + SE.toString());

			}catch (IOException IOE)
			{
				System.err.println("IOException: ProcessorSIMTRAC: Run: " + IOE.toString());
			}catch (Exception E)
			{
				System.err.println("Exception: ProcessorSIMTRAC: Run: " + E.toString());

			}finally
			{
				CerrarDB();
				CerrarArchivo();
				System.err.println("MSG: ProcessorSIMTRAC: Run: Finalizando Ejecución");
			}

		}
	}

	public ProcessorSIMTRAC(AppProperty tmpAP, int tempAño, int tempMes, int tempDia, int tempHora, int tempMinuto , int tmpMinutosAtras )
	{
		try {

			this.setName(this.getName() + ": ProcessorSIMTRAC");
			...
			{
				System.err.println("ERROR: ProcessorSIMTRAC: Parametros no validos " + FechaHora + " : " + MinutosAtras );
				return;
			}
			...
			System.err.println("MSG: ProcessorSIMTRAC: Inicializado con parametro " + FechaHora + " : " + MinutosAtras ); 
		

			this.start();

		} catch (Exception e) {
			System.err.println("Exception: ProcessorSIMTRAC: " + e.toString());
			DBParameters = null;
		}
